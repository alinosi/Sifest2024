...
class EssayCompeController extends Controller
{
    public function create() {

        // $user = EssayCompe::where('email', Auth::user()->email)->first();
        // $count_user = EssayCompe::all()->count();
        // $competition = ListCompetition::where('competition', 'EssayCompe')->first();
        ...
        // if ($user) {
        //     return back()->with("error", "You have registered for the Essay Competition");
        // }
    ...
    public function registration (Request $request) {
        ...
        if($request->file('idcard1') && $request->file('payment')){
            ...
            $EssayCompe = new EssayCompe($validation);
    ...
    public function submission(Request $request) {
        ...
        if ($user) {
            ...
            if ($request->file('submission')) {
                ...
                if ($user->submission) {
                    $old_path = public_path('EssayCompe\\').$user->submission;
                ...
                $submission->move(public_path('EssayCompe'), $filesubmission);
